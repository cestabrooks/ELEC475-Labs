...
def train(model, optimizer, n_epochs, loss_fn, data_laoder, device, plot_file, save_file):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        for imgs, labels in data_laoder:
            ...
            # Create a one_hot tensor of the output labels
            one_hot = torch.nn.functional.one_hot(labels, 10).to(torch.float32)
...
if __name__ == "__main__":
    ...
    train(model, torch.optim.Adam(model.parameters(), lr=0.001), 20, torch.nn.MSELoss(), data_loader, device, loss_plot, save_pth)
